/**
 * League of Legends — Account v1 (regional) + League v4 (plataforma).
 * A chave `X-Riot-Token` só é usada no servidor.
 */
import {
  platformRiotApiBaseUrl,
...
export async function fetchRankByRiotId(
  ...
  const accUrl = `${regionalRiotApiBaseUrl()}${accPath}`
  ...
  const leagueBase = platformRiotApiBaseUrl(platformRouting || 'br1')
